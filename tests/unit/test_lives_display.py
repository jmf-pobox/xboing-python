...
def patch_pygame(monkeypatch):
    ...
    monkeypatch.setattr("pygame.transform.smoothscale", lambda img, size: mock_ball_image)
...
def test_render_surface_size_and_visibility(monkeypatch, patch_pygame):
    ...
    assert found, f"Expected surface of size {(expected_width, expected_height)} to be created"
...
def test_fallback_empty_surface(monkeypatch):
    ...
    monkeypatch.setattr("pygame.image.load", lambda path: (_ for _ in ()).throw(FileNotFoundError()))

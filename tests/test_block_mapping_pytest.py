...
from src.game.sprite_block import SpriteBlockManager, SpriteBlock
...
def test_level01_loads_successfully(setup_level_manager):
    ...
    # Check level title
    assert level_manager.level_title == "Genesis", f"Expected level title 'Genesis', got '{level_manager.level_title}'"
    
    # Check time bonus
    assert level_manager.time_bonus == 120, f"Expected time bonus 120, got {level_manager.time_bonus}"
...
def test_level01_block_mapping(setup_level_manager):
    ...
    for block_type, expected_count in expected_blocks.items():
        ...
        assert block_types[block_type] == expected_count, \
...
def test_char_to_block_type_mapping():
    ...
    for char in test_chars:
        if char == '.':
            ...
            assert level_manager.CHAR_TO_BLOCK_TYPE.get(char) is None, \
                f"Character '{char}' should map to None"
        else:
            # Other characters should map to valid block types
            assert char in level_manager.CHAR_TO_BLOCK_TYPE, \

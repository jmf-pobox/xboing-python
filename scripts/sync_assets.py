...
CONVERTER_SCRIPT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'better_xpm_converter.py')
...
def copy_sounds():
    ...
    for au_file in au_files:
        ...
        if os.path.exists(src_wav):
            ...
            print(f"Copied: {os.path.basename(src_wav)} -> {os.path.basename(dest_wav)}")
        else:

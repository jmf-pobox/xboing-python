...
def parse_xpm(xpm_file):
    ...
    try:
        ...
        print(
            f"DEBUG: Parsed header: width={width}, height={height}, colors={num_colors}, chars_per_pixel={chars_per_pixel}"
        )
    ...
    print(
        f"DEBUG: Pixel data starts at index {pixel_start_index} (line {pixel_start_index+1})"
    )
    print(
        f"DEBUG: Color table has {len(color_table)} entries: {list(color_table.keys())}"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert XBoing XPM files to PNG format"
    )
    parser.add_argument("input", help="Input XPM file or directory")

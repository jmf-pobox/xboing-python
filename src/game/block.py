...
class Block:
    ...
    def __init__(self, x, y, width, height, block_type=TYPE_NORMAL, color_name='blue', points=100):
        """
        Initialize a block.
        ...
        # For unbreakable blocks, add a different indicator
            ...
            for y_offset in range(5, self.height - 5, 4):
                pygame.draw.line(surface, (255, 255, 255),
                               ...
                               (self.rect.right - 5, self.rect.top + y_offset), 1)
...
class BlockManager:
    ...
    def __init__(self, brick_width=50, brick_height=20, margin=2, offset_x=0, offset_y=0):
        """
        ...
        if level_num == 1:
            ...
            for row in range(rows):
                for col in range(blocks_per_row):
                    x = self.offset_x + self.margin + col * (self.brick_width + self.margin)
                    y = self.offset_y + top_margin + row * (self.brick_height + self.margin)
                    
                    # Alternate colors
                    colors = ['blue', 'red', 'green', 'yellow']
                    color = colors[row % len(colors)]
                    
                    block = Block(x, y, self.brick_width, self.brick_height, 
                                 Block.TYPE_NORMAL, color)

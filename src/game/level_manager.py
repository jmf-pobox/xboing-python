...
class LevelManager:
    ...
    def _create_blocks_from_layout(self, layout):
        ...
        total_width = (max_cols * block_width) + ((max_cols - 1) * horizontal_spacing) + (2 * wall_spacing)
        ...
        for row_idx, row in enumerate(layout):
            for col_idx, char in enumerate(row):
                ...
                x = self.block_manager.offset_x + left_margin + col_idx * (block_width + horizontal_spacing)
                
                # Add top margin for vertical positioning with 50% block height spacing
                top_margin = wall_spacing
                y = self.block_manager.offset_y + top_margin + row_idx * (brick_height + vertical_spacing)
                ...
                if char in '12345':  # Counter blocks 1-5
                    ...
                    block.image_file = 'cntblk.png'  # The base counter block image without a number
    ...
    def _set_level_background(self):
        ...
        # Debug information
        print(f"Setting level {self.current_level} background to: {bg_file} (background {self.current_background})")

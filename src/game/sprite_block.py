...
class SpriteBlock:
    ...
    ANIMATION_FRAMES = {
        TYPE_COUNTER: ['cntblk.png', 'cntblk1.png', 'cntblk2.png', 'cntblk3.png', 'cntblk4.png', 'cntblk5.png'],
        TYPE_DEATH: ['death1.png', 'death2.png', 'death3.png', 'death4.png', 'death5.png'],
    ...
    XBOING_COLORS = {
        ...
        TYPE_GREEN: [(60, 240, 60), (30, 200, 30), (30, 150, 30)],     # Brighter green with visible shadow
    ...
    def preload_images(cls, blocks_dir):
            ...
            if isinstance(frames, list):
                for frame in frames:
                    ...
                    if os.path.exists(image_path) and frame not in cls._image_cache:
                        try:
                            cls._image_cache[frame] = pygame.image.load(image_path).convert_alpha()
                            loaded_count += 1
                        except Exception as e:
        ...
        print(f"Block image loading complete: {loaded_count} loaded, {failed_count} failed")
    ...
    def __init__(self, x, y, block_type=TYPE_BLUE):
        ...
        if block_type in self.BLOCK_IMAGES:
            ...
            self.image_file, self.points, self.behavior = self.BLOCK_IMAGES[self.TYPE_BLUE]
        ...
        if self.image_file in self._image_cache:
            ...
            print(f"Error: Missing block image '{self.image_file}' for block type {block_type}")
            self.image = pygame.Surface((self.width, self.height), pygame.SRCALPHA)
            pygame.draw.rect(self.image, (255, 0, 255), pygame.Rect(0, 0, self.width, self.height))
    ...
    def _create_block_image(self, block_type):
        ...
        highlight_color = (min(255, main_color[0]+50), min(255, main_color[1]+50), min(255, main_color[2]+50))
    ...
    def update(self, delta_ms):
        ...
        if self.animation_frames and not (self.type == self.TYPE_COUNTER and hasattr(self, 'counter_value') and self.counter_value == 0):
    ...
    def hit(self):
        ...
        if self.behavior == self.BEHAVIOR_UNBREAKABLE:
            ...
            if self.health > 0 and self.animation_frames:
                ...
                frame_index = max(0, min(5 - self.health, len(self.animation_frames) - 1))
                if self.image_file in self._image_cache:
...
class SpriteBlockManager:
    ...
    def create_level(self, level_num=1, width=495, top_margin=60):
        ...
        left_margin = wall_spacing + (usable_width - (max_cols * block_width + total_spacing)) // 2
        ...
        if level_num == 1:
            ...
            for row in range(max_rows):
                for col in range(max_cols):
                    ...
                    y = self.offset_y + top_margin + row * (self.brick_height + self.vertical_spacing)
            ...
            block_types = [
                SpriteBlock.TYPE_RED, SpriteBlock.TYPE_BLUE, SpriteBlock.TYPE_GREEN,
                SpriteBlock.TYPE_TAN, SpriteBlock.TYPE_YELLOW, SpriteBlock.TYPE_PURPLE,
                SpriteBlock.TYPE_BOMB, SpriteBlock.TYPE_EXTRABALL, SpriteBlock.TYPE_MULTIBALL,
                SpriteBlock.TYPE_STICKY, SpriteBlock.TYPE_REVERSE, SpriteBlock.TYPE_BONUS,
                SpriteBlock.TYPE_BONUSX2, SpriteBlock.TYPE_TIMER, SpriteBlock.TYPE_MGUN
            ]
            ...
            for row in range(max_rows):
                for col in range(max_cols):
                    ...
                    r = random.random()
                    if row == 0 and r < 0.3:
                        block_type = SpriteBlock.TYPE_BLACK  # Some unbreakable blocks in top row
                    elif r < 0.05:
                        # Rare blocks
                        rare_blocks = [
                        ...
                        # Common blocks
                        block_type = random.choice(block_types[:6])  # Regular colored blocks

...
class Ball:
    ...
    def __init__(self, x, y, radius=8, color=(255, 255, 255)):
        ...
        self.sprite_index = random.randint(0, len(Ball.sprites)-1) if Ball.sprites else 0
    ...
    def update(self, delta_ms, screen_width, screen_height, paddle=None, offset_x=0, offset_y=0):
        """
    ...
    def _add_random_factor(self):
        ...
        speed = math.sqrt(self.vx * self.vx + self.vy * self.vy)
        angle = math.atan2(-self.vy, self.vx)  # Note negative vy due to screen coordinates

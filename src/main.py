...
from src.engine.graphics import Renderer, Sprite
...
from src.utils.asset_loader import load_image, create_font
from src.utils.audio_player import DirectAudioPlayer
from src.utils.background_generator import create_pattern_background, create_grid_background
from src.utils.layout import GameLayout
from src.utils.asset_paths import get_sounds_dir, get_asset_path
...
def main():
    ...
    while running:
        ...
        if game_over:
            ...
            renderer.draw_text("GAME OVER", font, (255, 50, 50), 
                               play_rect.centerx, play_rect.centery - 30, centered=True)
            renderer.draw_text(f"Final Score: {score}", font, (255, 255, 255), 
                               play_rect.centerx, play_rect.centery + 10, centered=True)
            renderer.draw_text("Press R to restart", small_font, (200, 200, 200), 
                               play_rect.centerx, play_rect.centery + 50, centered=True)
        ...
        if input_manager.is_key_down(pygame.K_PLUS) or input_manager.is_key_down(pygame.K_KP_PLUS) or input_manager.is_key_down(pygame.K_EQUALS):
            # Increase volume
            sound_volume = min(1.0, sound_volume + 0.1)
            # Update volume for all sounds
            for sound_name, base_volume in sound_files.items():
                if sound_name in sounds:
                    sounds[sound_name].set_volume(base_volume * (sound_volume if sound_enabled else 0))
            play_sound('click')
        ...
        for ball in balls:
            ...
            if is_active:
                ...
                for effect in effects:
                    ...
                    elif effect in [SpriteBlock.TYPE_PAD_EXPAND, SpriteBlock.TYPE_PAD_SHRINK]:
                        # Change paddle size
                        if effect == SpriteBlock.TYPE_PAD_EXPAND:
                            paddle.width = min(PADDLE_WIDTH * 1.5, PADDLE_WIDTH * 2)  # Expand by 50%
                        else:
                            paddle.width = max(PADDLE_WIDTH * 0.5, PADDLE_WIDTH / 2)  # Shrink by 50%
                ...
                if hit_wall:
                    ...
                    if 'boing' in sounds:
                        ...
                        sounds['boing'].set_volume(temp_vol * 0.2)  # ~20% of normal volume
                        sounds['boing'].play()
        ...
        pygame.draw.rect(window.surface, wall_color, 
                         pygame.Rect(play_rect.x, play_rect.y, play_rect.width, 2))  # Top
        pygame.draw.rect(window.surface, wall_color, 
                         pygame.Rect(play_rect.x, play_rect.y, 2, play_rect.height))  # Left
        pygame.draw.rect(window.surface, wall_color, 
                         pygame.Rect(play_rect.x + play_rect.width - 2, play_rect.y, 2, play_rect.height))  # Right
        ...
        # Score area
        renderer.draw_text(f"Lives: {lives}", font, (255, 255, 255), 
                           score_rect.x + 20, score_rect.y + 15)
        renderer.draw_text(f"Score: {score}", font, (255, 255, 255), 
                           score_rect.x + score_rect.width - 80, score_rect.y + 15)
                          
        # Level info
        level_info = level_manager.get_level_info()
        renderer.draw_text(f"Level: {level_info['level_num']}", font, (255, 255, 255), 
                           level_rect.centerx, level_rect.y + 15, centered=True)
        renderer.draw_text(f"{level_info['title']}", small_font, (200, 200, 255), 
                           level_rect.centerx, level_rect.y + 40, centered=True)
        ...
        if not waiting_for_launch and not game_over and not level_complete:
            ...
            renderer.draw_text(f"Time: {time_remaining}", small_font, time_color, 
                               level_rect.centerx, level_rect.y + 60, centered=True)
        ...
        if level_complete:
            ...
            level_overlay = pygame.Surface((play_rect.width, play_rect.height), pygame.SRCALPHA)
            level_overlay.fill((0, 0, 0, 150))  # Semi-transparent black
            window.surface.blit(level_overlay, (play_rect.x, play_rect.y))
            
            renderer.draw_text("LEVEL COMPLETE!", font, (50, 255, 50), 
                              play_rect.centerx, play_rect.centery - 30, centered=True)
            renderer.draw_text("Press SPACE for next level", small_font, (200, 200, 200), 
                              play_rect.centerx, play_rect.centery + 10, centered=True)
